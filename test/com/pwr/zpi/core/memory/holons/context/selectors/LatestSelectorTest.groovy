 ...
 */
class LatestSelectorTest extends GroovyTestCase {
    ...
    void testSelect() {
        namedGroundingSets = Grounder.getGroundingSets(formula, [bp1, bp2, bp3, bp4, bp5] as Set<BaseProfile>)
        latestSelector = new LatestSelector()
        def res = latestSelector.select(namedGroundingSets)
        assertTrue(res.size()==1)
        assertEquals([bp5] as Set<BaseProfile>, res)
        ...
        res = latestSelector.select(namedGroundingSets)
        assertTrue(res.size()==1)
        assertEquals([bp8] as Set<BaseProfile>,res)

        namedGroundingSets = Grounder.getGroundingSets(formula, [bp1, bp2, bp3, bp4, bp5, bp6] as Set<BaseProfile>)
        assertTrue(latestSelector.select(namedGroundingSets).containsAll([bp6] as Set<BaseProfile>)
            || latestSelector.select(namedGroundingSets).containsAll([bp5] as Set<BaseProfile>)) //non-deterministic for BPs with same timestamp

        shouldFail {
            latestSelector.select([] as Map<Formula, Set<BaseProfile>>)
        }
    }
}
